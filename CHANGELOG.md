...
- Replace per-driver replay-bootstrap with chunked all-drivers fetches in `f1core::polling::bootstrap_session_data` (`crates/f1core/src/polling.rs`)
  - Symptom diagnosed against the local f1-pitwall web DB: every recent race opened in the web app (Canada, Singapore, Brazil, Mexico, USA, Qatar, UAE, …) had a fully-populated `drivers` / `laps` / `positions` / `stints` set but **zero rows** in `car_data` and `location`. Only one historical session (China, March 2026) had telemetry. Downstream: `board::build_race_board`'s retirement heuristic — "no real car_data in the last 45 s" — flagged ~half the field as DNF on lap 3 of every race, because the heuristic was reading an empty table rather than missing recent samples
  - Root cause: the existing `bootstrap_session_data` fanned out one full-session `get_car_data` + one full-session `get_location` request **per driver**, with `TELEMETRY_BOOTSTRAP_CONCURRENCY = 4`. A single driver's full-race `car_data` is ~3.9 MB / ~5.5 s on a warm cache (`curl https://api.openf1.org/v1/car_data?session_key=11291&driver_number=1&…` measured at 0.7 MB/s download). Run four of those concurrently and bandwidth contention pushes each above `reqwest`'s 10 s timeout (hard-coded in `OpenF1Client::with_rate_limit`) → request aborts → `bootstrap car_data fetch failed` log → silent per-driver return → zero rows written. The China race likely landed when network conditions were better and slipped through; every later race tripped the timeout for at least one driver and bailed before retrying. Per-driver completeness then said "0 rows = not complete," so reopens kept re-failing without ever filling the hole
  - Fix shape — borrow the strategy `pw` has used since 0.28.0 (`crates/pw/src/bootstrap.rs`, now reduced to a thin wrapper): split the session window into 15-minute chunks, and for each chunk fire one `get_car_data_all_drivers` + one `get_location` (with empty driver slice) in parallel. 15 min is the largest window OpenF1 will return for all-driver requests without 422'ing on payload size, and keeps each response under ~12 MB / ~7 s — comfortably inside the 10 s timeout regardless of per-driver bandwidth contention. For a 2 h race that's ~16 total requests instead of ~88, with each request statistically much more likely to succeed
  - Fix shape — retries: each chunk gets `BOOTSTRAP_CHUNK_RETRIES = 3` attempts with 1 s / 2 s / 4 s exponential backoff via `fetch_with_retry`. The dominant failure mode at chunk granularity is transient (decode errors mid-stream, occasional 5xx) — a couple of quick retries turns those into eventually-consistent loads instead of permanent gaps that survive across replay reopens
  - Fix shape — idempotency on reopen: a fast-path check walks all drivers and short-circuits the entire chunk loop when every driver already meets both `car_data_complete` and `location_complete` thresholds. Avoids walking the chunk list (and the per-chunk no-op upserts) on every reopen of a fully-loaded session. Mid-loop reopens are still safe — `upsert_car_data` / `upsert_location` are `INSERT OR IGNORE` on a PK that includes `date`, so re-running over a partially-loaded session fills holes without duplicating
  - New public API: `BootstrapProgress { completed, total }` + `bootstrap_session_data_with_progress<F>(..., on_progress: F)` for callers that want to surface a `Loading N/M` overlay. The plain `bootstrap_session_data(...)` is preserved as a thin wrapper that passes a no-op callback — the web's existing `ws.rs` callsite continues to compile unchanged
  - TUI catch-up: `crates/pw/src/bootstrap.rs` shrinks from a ~220-line standalone chunked impl to a ~45-line wrapper that owns the `Status` arc the loading overlay reads, and forwards `BootstrapProgress` callbacks into it. `Status` / `new_status` / `Progress` (now a re-export of `BootstrapProgress`) keep their existing shape, so `crates/pw/src/app/mod.rs` and `crates/pw/src/pages/session.rs` continue to compile without changes
  - Tests: 4 new tests in `crates/f1core/src/polling.rs` pin the chunking math. `splits_full_window_into_15min_chunks` — 2 h race → 8 chunks, first chunk starts at `date_start`, last chunk ends exactly at `date_end`. `final_chunk_is_clamped_to_end` — 20-min window splits into one 15-min chunk and one 5-min tail, neither overruns. `sub_chunk_window_yields_single_chunk` — sessions shorter than `BOOTSTRAP_CHUNK_SECS` get a single chunk spanning the whole window. `empty_window_yields_no_chunks` — defensive: collapsed bounds produce zero chunks, so the bootstrap is a no-op rather than spinning. All 79 existing tests continue to pass
...
- Fix best-sector / best-lap classification leaving freshly-set bests as `Normal` (yellow) instead of `PersonalBest` / `SessionBest` (`crates/f1core/src/db/queries.rs`)
  - Two compounding symptoms, both from `best_sectors_inner` / `best_lap_inner`. First: on lap 1 of every race, every driver's S1 (and S2, S3 as they elapsed) showed yellow on the leaderboard — including the field-leader's session-best S1 — until each driver finished their first lap. With no completed laps in the pool the per-driver PB and the overall session best were both `None`, and `classify_sector` falls through to `Normal` whenever a visible sector matches neither. Second: random yellow flashes on freshly-set fastest sectors mid-race, always on the most-recent completed lap of a driver still on their opening stint, self-healing one lap later
  - Root cause #1 — lap-completion gate vs. per-sector visibility. `visible_sectors` (the progressive reveal used by `race_display` / `qualifying_display`) gates each sector independently with millisecond precision: S1 visible iff `date_start + s1 <= clock_now`, S2 iff `+s1+s2`, S3 iff `+s1+s2+s3`. `best_sectors_inner` instead required the *whole lap* to be complete (`lap_duration IS NOT NULL AND lap_duration > 0` plus a `date_start + lap_duration + 1s <= clock_now` check with a 1-second safety buffer). So a sector could be visible — and a value displayed for it — for up to a full lap before it became eligible to own purple / green. The first-lap-of-the-race symptom is the extreme case of the same shape: no lap has finished, so nothing is eligible, so everything is yellow
  - Root cause #2 — open-stint latest lap silently dropped. Both queries ended with `AND (s.lap_end IS NULL OR l.lap_number != s.lap_end)` against a `LEFT JOIN stints` on `lap_number BETWEEN lap_start AND lap_end`. The intent was "skip the in-lap of a closed stint." But OpenF1's stints endpoint advances `lap_end` to *the latest completed lap of an open (live) stint* — the same trap that `get_race_board_rows` / `get_qualifying_board_rows` already gate around with their `closed_in_laps` CTE. So for any driver who hadn't pitted yet, the filter threw away their most-recent completed lap on every snapshot tick, leaving the previous lap's PB stale and any new-best sector classified as `Normal`. The mis-classification cleared the moment the next lap completed (lap_end advanced again), which is why it manifested as a roughly-one-lap-long yellow flash rather than a permanent state
  - Fix: per-sector eligibility now mirrors `visible_sectors` exactly. Each of the three sector MIN aggregates is wrapped in a CASE WHEN that checks `julianday(date_start) + Σsᵢ / 86400.0 <= julianday(clock_now)` — sub-second precision, no buffer, no dependence on lap completion. `julianday` is used instead of the `datetime(..., '+N seconds')` modifier so the comparison can carry fractional seconds without the `CAST(... AS INTEGER) + 1` rounding that the old shape needed. Drops `l.lap_duration IS NOT NULL` from the WHERE clause entirely for `best_sectors_inner` (each sector now self-gates). `best_lap_inner` keeps the lap-completion gate (a lap_duration must exist before there's a lap to consider best) but its `datetime(...) <= datetime(?)` cap with 1s buffer stays — that gate already matches the moment `vis_s3` flips visible, since both depend on `date_start + lap_duration` having elapsed
  - Fix part 2: replace the brittle `l.lap_number != s.lap_end` exclusion with a real "stint is actually closed" gate — a `closed_in_laps` CTE that requires either a successor stint or a pit_stops row at lap_end (same shape as the board-row queries). Both `best_lap_inner` and `best_sectors_inner` use it. Open-stint drivers' latest completed laps now contribute to PB; properly-closed stint in-laps stay excluded (their S3 is contaminated by pit-entry braking)
  - Out-laps remain excluded by design (`is_pit_out_lap=0` filter unchanged). They are the only laps where the *first* sector is contaminated rather than the last, and the contamination magnitude varies enough with pit-exit timing that they are not worth trying to partially salvage
  - Tests: 7 new tests in `crates/f1core/src/db/queries.rs`. `best_sectors_includes_in_progress_lap_one_s1` reproduces the screenshot scenario — three drivers on lap 1 with only S1 elapsed — and asserts the leader owns purple S1 and every driver's per-driver PB equals their own value. `best_sectors_per_sector_time_gate_excludes_future_sectors` pre-loads all three sectors on a single lap and advances `clock_now` in three stages, asserting each sector flips eligible at exactly its elapsed-time boundary. `best_sectors_open_stint_latest_lap_counts_toward_pb` pins root cause #2: two laps on stint 1 with `lap_end=2`, the faster lap must own PB sectors *and* PB lap. `best_sectors_closed_stint_in_lap_still_excluded` adds a successor stint and asserts the original in-lap stays out of the pool. `best_sectors_replay_future_lap_excluded` pre-loads a faster lap in the replay future and asserts none of its sectors leak through the per-sector time gate. `best_sectors_out_lap_excluded_by_design` plants a contaminated S1 on an out-lap and asserts the slower regular-lap S1 wins. `best_lap_in_progress_first_lap_returns_none` pins the contract that `get_best_lap` still requires a finished lap, even after the open-stint CTE rewrite

## 0.35.2

- Stop the residual snapshot-tick CPU creep from laps/stints scans (`crates/f1core/src/db/queries.rs`)
  - After 0.35.1 flattened the positions/intervals scan, a residual ~0.06 %/min CPU climb remained in long replays — visible on the Fly graph as a slow upward slope from ~3.3 % at attach to ~5 % over 30 min (no `ticker_timings` rebaseline session yet, but the shape matched). Two CTEs in `get_race_board_rows` were still scan-shaped: `ranked_laps` (`ROW_NUMBER() OVER (PARTITION BY driver_number ORDER BY lap_number DESC)` over every per-session lap row up to `clock_now`, used twice for latest + prev lap) and `ranked_stints` (same shape over `stints`, gated by a `driver_max_lap` CTE that itself did a `GROUP BY` over all laps). `pit_counts` joined `ranked_laps` and inherited the scan. `laps` and `stints` are much smaller than `positions`/`intervals` in absolute terms, but they were the dominant remaining scan, and the cost grew with race progression
  - Fix: same per-driver index-seek pattern that handled positions/intervals in 0.35.1, now applied to the `(session_key, driver_number, lap_number)` PK on `laps` and the `(session_key, driver_number, stint_number)` PK on `stints`. The four `LEFT JOIN ranked_*` instances become four direct correlated joins with `lap_number = (SELECT MAX(l2.lap_number) … WHERE date_start <= ?2)` / `stint_number = (SELECT MAX(s2.stint_number) … WHERE lap_start <= COALESCE(l.lap_number, 1))` shapes. The prev-row joins add `lap_number < l.lap_number` / `stint_number < st.stint_number` to the same correlated subquery. `driver_max_lap` is inlined as the same `MAX(lap_number)` lookup the latest-lap join already does, then referenced as `l.lap_number` in the stint joins — one source of truth for "driver's current lap by `clock_now`". `pit_counts` collapses to a scalar `COUNT(*)` subquery directly in the SELECT clause, using `l.lap_number` from the outer query
  - Kept as-is (small absolute row count, scans don't grow meaningfully with replay duration): `closed_in_laps` (`stints` + EXISTS on `stints`/`pit_stops`), `stopped_drivers` (`race_control`), `pit_status` / `latest_pit` (`pit_stops`)
  - Tests: 5 new regression tests pin the rewritten joins. `race_board_picks_latest_and_prev_lap` + `race_board_excludes_lap_rows_in_replay_future` cover the laps joins (latest + prev, replay-future bound); `race_board_picks_latest_and_prev_stint` + `race_board_excludes_future_stints` cover the stints joins (including the `lap_start <= driver_max_lap` gate that the inlined `l.lap_number` provides); `race_board_pit_count_excludes_current_lap` pins the inline `pit_count` subquery's `lap_number < l.lap_number` strict-inequality. All 70 existing tests continue to pass

## 0.35.1

- Stop snapshot-tick cost growing linearly with replay duration (`crates/f1core/src/db/queries.rs`)
  - Symptom from a partial Singapore 2025 replay run, observed via the pitwall `ticker_timings` log + Fly CPU graph: snapshot build p50 climbed from 32 ms at t=0 to 73 ms at t≈15 min of replay clock — a near-perfectly linear curve at ~2.7 ms/min. The downstream effect on Fly was VM CPU rising from ~5 % to ~20 % over 20 min, with no plateau in sight. Every connected replay socket runs its own 200 ms ticker, so a single long replay session was enough to saturate `shared-cpu-2x` and starve other connections
  - Root cause: `get_race_board_rows` opened with two CTEs — `ranked_positions` and `ranked_intervals` — each of which did `SELECT *, ROW_NUMBER() OVER (PARTITION BY driver_number ORDER BY date DESC) FROM <table> WHERE session_key=?1 AND (date IS NULL OR date <= ?2)`, joined into the outer query with `rn=1`. The outer query only ever reads each driver's latest row, but the CTE had to *materialize and sort every row up to `clock_now`* before the window filter could discard the rest. `positions` and `intervals` are written by OpenF1 ingest at multi-Hz × ~20 drivers, so each table grows to tens of thousands of rows mid-race — and the CTE cost scaled linearly with that count, on every 200 ms snapshot tick. `get_interval_history` had the same shape (`PARTITION BY driver_number ORDER BY date DESC` then `WHERE rn <= ?3`), scanning the full filtered `intervals` set every tick just to keep 12 entries per driver
  - Fix: replace both CTEs in `get_race_board_rows` with correlated `LEFT JOIN`s on `positions` / `intervals` whose ON clause carries `p.date = (SELECT MAX(p2.date) FROM positions p2 WHERE p2.session_key=d.session_key AND p2.driver_number=d.driver_number AND p2.date IS NOT NULL AND p2.date <= ?2)`. The `(session_key, driver_number, date)` PK lets SQLite resolve each subquery as an index seek to the end of the per-driver range — ~20 `O(log N)` lookups per snapshot tick instead of one `O(N)` scan + window sort. `get_interval_history` is reshaped the same way: one `SELECT driver_number FROM drivers` then a per-driver `ORDER BY date DESC LIMIT ?` against the PK index, then reverse-in-Rust to preserve the previous chronological output order. Per-tick cost is now flat in replay duration
  - Behavior trade: the original CTEs also surfaced rows with `date IS NULL` (`(date IS NULL OR date <= ?2)`). The new shape filters those out — OpenF1 always populates `date` on `positions` and `intervals` (the column is part of the PK), so the defensive branch never fired in production
  - Tests: 3 new regression tests pin the latest-per-driver behavior. `race_board_picks_latest_position_and_interval_per_driver` inserts two rows per driver and asserts the newer one wins; `race_board_excludes_position_rows_in_replay_future` moves `clock_now` between the rows and asserts the older one wins; `interval_history_returns_latest_n_chronological` inserts 5 rows + 1 future row, requests `max_entries=3`, and asserts the latest 3 are returned in oldest-first order. The existing `race_open_stint_does_not_flag_in_lap` / `race_closed_stint_flags_in_lap` continue to pass under the rewritten joins
  - Follow-ups deferred to a second pass (still scan-shaped but small in absolute row count, and trickier to rewrite without changing semantics): `pit_status` / `latest_pit` on `pit_stops`, `stopped_drivers` on `race_control`, `closed_in_laps` on `stints`
...
- Fix qualifying flying laps revealing in full the moment they start (`crates/f1core/src/domain/sector.rs`, `crates/f1core/src/util/time.rs`)
  - Symptom from a Miami 2026 Sprint Qualifying replay: NOR's lap 13 (out lap, stint 3) correctly showed the "OUT" overlay, then ~10s into lap 14 (the SQ3 hot lap on softs) the leaderboard already displayed the full final time `1:27.869` plus all three sector splits `29.682 / 33.433 / 24.754` — the lap had only been running for 10s of its eventual 88s. Every flying lap was effectively spoiling its own time the instant the clock crossed the lap's `date_start`
  - Root cause: `visible_sectors` (the progressive-reveal gate used by `qualifying_display`) parsed `lap_date_start` with `d.parse::<DateTime<Utc>>().ok()`. `DateTime::<Utc>::from_str` requires an RFC 3339 timezone suffix, but OpenF1 writes `laps.date_start` as naive ISO with no offset (e.g. `"2026-05-01T21:11:45.087000"`), so the parse always returned `None` for replay rows. The `None` branch comments "no date info, show all" and unconditionally returns `(s1, s2, s3)` — meaning `vis_s3.is_some()` was *always true* for any lap with all three sector durations populated, and `qualifying_display`'s `if vis_s3.is_some() { b.last_lap } else { b.prev_last_lap }` always chose `b.last_lap`. Out-laps and in-laps were unaffected because `qualifying_display` short-circuits on `is_throwaway = is_pit_out_lap || is_in_lap` before calling `visible_sectors`; the leak only manifested once the lap was no longer flagged as throwaway, i.e. the very moment the flying lap began
  - Fix: route `visible_sectors` through the project-wide `crate::util::time::parse_ts` helper, and add a third fallback to `parse_ts` for naive ISO without offset (`%Y-%m-%dT%H:%M:%S%.f`), treating it as UTC. The "no date info, show all" branch is preserved as a genuine last resort for callers that pass `None` or a malformed string. `parse_ts` now accepts all three OpenF1-shaped forms: RFC 3339 with offset, `+00:00`-suffixed, and bare naive ISO
  - In live mode the bug was latent rather than visible: OpenF1's `Lap` model has every duration field as `Option<f64>` and the upsert merges fields, so sectors arrive progressively over MQTT and `visible_sectors` was effectively gated by message arrival order rather than the wall clock. The progressive reveal it was *supposed* to provide had simply never been working — replay just exposed it because all data is bootstrapped at session attach
  - Tests: `naive_iso_treated_as_utc` pins the new `parse_ts` fallback; `naive_utc_date_start_gates_progressive_reveal` reproduces the exact NOR scenario (lap start `2026-05-01T21:11:45.087000`, clock `21:11:53` → `(None, None, None)`, then `21:12:16` → `(s1, None, None)`, then `21:13:14` → `(s1, s2, s3)`) so any future regression in either the parser or the gate fails the suite
